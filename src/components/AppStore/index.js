...
class AppStore extends Component {
  ...
  getTabDetails = appSearchResults => {
    const {activeTabId} = this.state
    const tabAppList = appSearchResults.filter(
  ...
  render() {
    ...
    const appSearchResults = this.getSearchResults()
    const filteredTabDetails = this.getTabDetails(appSearchResults)
        ...
        <ul className="tabs-list-container">
          {tabsList.map(eachTab => (
            <TabItem
              tabItem={eachTab}
              key={eachTab.tabId}
              getActiveTabApps={this.getActiveTabApps}
              isActive={eachTab.tabId === activeTabId}
            />
          ))}
        </ul>
        <ul className="apps-list-container">
          {filteredTabDetails.map(eachAppList => (
